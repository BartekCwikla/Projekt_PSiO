    ...
    void Game::update(sf::Time& dt) {
        ...
        //Damage enemy if it's in melee weapon range
        if (player.getMelee() != nullptr) {
            for (auto& e : enemies) {
                float dist = std::hypot(player.getPosition().x - e->getPosition().x,
                                        player.getPosition().y - e->getPosition().y);
                if (dist <= player.getMelee()->getRange()) {
                    e->takeDamage(player.getMelee()->getDamage());
                }
            }
        }
